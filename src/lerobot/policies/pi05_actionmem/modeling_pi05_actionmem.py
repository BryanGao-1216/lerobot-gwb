...
from .tokenization_pi05_actionmem import PI05ActionMemTokenMap
...
def _configure_action_vqvae_flow_normalization(
    ...
    if action_normalization is NormalizationMode.IDENTITY:
        ...
        config.action_vqvae_flow_q01 = None
    ...
    if action_normalization is NormalizationMode.MEAN_STD:
        first = (
            ...
            else config.action_vqvae_flow_mean
        )
        second = (
            runtime_action_stats.get("std")
            if runtime_action_stats is not None
            else config.action_vqvae_flow_std
        )
        names = ("mean", "std")
    elif action_normalization is NormalizationMode.QUANTILES:
        first = (
            runtime_action_stats.get("q01")
            if runtime_action_stats is not None
            else config.action_vqvae_flow_q01
        )
        second = (
            runtime_action_stats.get("q99")
            if runtime_action_stats is not None
            else config.action_vqvae_flow_q99
        )
        names = ("q01", "q99")
    else:
        raise ValueError(
            "PI05ActionMem VQ-VAE flow-source normalization supports ACTION modes "
            f"IDENTITY, MEAN_STD, and QUANTILES, got {action_normalization.value}."
        )

    if first is None or second is None:
        raise ValueError(
            f"PI05ActionMem uses {action_normalization.value} action normalization, but "
            f"{names[0]}/{names[1]} are unavailable from {source}."
        )
    first_tensor = torch.as_tensor(first, dtype=torch.float32).detach().cpu()
    second_tensor = torch.as_tensor(second, dtype=torch.float32).detach().cpu()
    if first_tensor.ndim != 1 or second_tensor.ndim != 1 or first_tensor.shape != second_tensor.shape:
        raise ValueError(
            f"Action {names[0]}/{names[1]} from {source} must be same-shaped 1D tensors, got "
            f"{tuple(first_tensor.shape)} and {tuple(second_tensor.shape)}."
        )
    if not torch.isfinite(first_tensor).all() or not torch.isfinite(second_tensor).all():
        raise ValueError(f"Action {names[0]}/{names[1]} from {source} must be finite.")
    if action_normalization is NormalizationMode.MEAN_STD and torch.any(second_tensor < 0):
        raise ValueError("Action std must be non-negative.")
    if action_normalization is NormalizationMode.QUANTILES and torch.any(second_tensor < first_tensor):
        raise ValueError("Action q99 must be greater than or equal to q01.")

    if action_normalization is NormalizationMode.MEAN_STD:
        config.action_vqvae_flow_mean = first_tensor.tolist()
        config.action_vqvae_flow_std = second_tensor.tolist()
        config.action_vqvae_flow_q01 = None
        config.action_vqvae_flow_q99 = None
    else:
        config.action_vqvae_flow_q01 = first_tensor.tolist()
        config.action_vqvae_flow_q99 = second_tensor.tolist()
        config.action_vqvae_flow_mean = None
        config.action_vqvae_flow_std = None
...
class PI05ActionMemPytorch(nn.Module):  # see openpi `PI0Pytorch`
    ...
    def __init__(self, config: PI05ActionMemConfig, rtc_processor: RTCProcessor | None = None):
        ...
        self.action_token_map = PI05ActionMemTokenMap.from_json(config.action_token_map_path)
        config.action_token_map_path = self.action_token_map.path
        if config.chunk_size != self.action_token_map.action_horizon:
            raise ValueError(
                f"PI05ActionMem chunk_size ({config.chunk_size}) must match the VQ-VAE action horizon "
        ...
        if config.max_action_dim < self.action_token_map.action_dim:
            raise ValueError(
                f"PI05ActionMem max_action_dim ({config.max_action_dim}) must be at least the VQ-VAE "
        ...
        if self._action_normalization is NormalizationMode.MEAN_STD:
            ...
            stat_names = "mean/std"
        elif self._action_normalization is NormalizationMode.QUANTILES:
            first, second = config.action_vqvae_flow_q01, config.action_vqvae_flow_q99
            stat_names = "q01/q99"
        else:
            first, second = None, None
            stat_names = ""

        if self._action_normalization is not NormalizationMode.IDENTITY:
            if first is None or second is None:
                raise ValueError(
                    f"PI05ActionMem {self._action_normalization.value} flow-source normalization "
                    f"requires {stat_names} in the config."
                )
            if len(first) != self.action_token_map.action_dim:
                raise ValueError(
                    f"PI05ActionMem action {stat_names} dimension ({len(first)}) must match the "
                    f"VQ-VAE action dimension ({self.action_token_map.action_dim})."
                )
            self.register_buffer(
                "_action_vqvae_flow_stat_a", torch.tensor(first, dtype=torch.float32), persistent=False
            )
            self.register_buffer(
                "_action_vqvae_flow_stat_b", torch.tensor(second, dtype=torch.float32), persistent=False
            )
        else:
            self.register_buffer("_action_vqvae_flow_stat_a", None, persistent=False)
            self.register_buffer("_action_vqvae_flow_stat_b", None, persistent=False)

        checkpoint_path = config.action_vqvae_checkpoint_path or self.action_token_map.vqvae_checkpoint_path
        if checkpoint_path is None:
            raise ValueError(
                "Set action_vqvae_checkpoint_path in PI05ActionMemConfig or "
                "vqvae.checkpoint_path in the PI05ActionMem token map."
            )
        ...
        if action_vqvae is None:
            ...
            if action_vqvae.horizon != self.config.chunk_size:
                raise ValueError(
                    ...
                    f"PI05ActionMem chunk_size ({self.config.chunk_size})."
                )
    ...
    def decode_action_tokens(self, action_tokens: Tensor) -> Tensor:
        ...
        if self._action_normalization is NormalizationMode.MEAN_STD:
            ...
            denominator = self._action_vqvae_flow_stat_b - self._action_vqvae_flow_stat_a
            denominator = torch.where(
                denominator == 0,
                torch.full_like(denominator, self.config.action_vqvae_flow_normalization_eps),
                denominator,
            )
            decoded_actions = 2.0 * (decoded_actions - self._action_vqvae_flow_stat_a) / denominator - 1.0
    ...
    def _validate_action_token_sequence(self, action_tokens, action_token_masks) -> None:
        if action_tokens is None or action_token_masks is None:
            raise ValueError("PI05ActionMem requires action_tokens and action_token_masks.")
        ...
        if action_tokens.shape[1] < 2:
            raise ValueError("PI05ActionMem requires ACTION_QUERY plus a reserved action-token target.")
        ...
        if torch.any(invalid_query):
            raise ValueError(
                "The penultimate PI05ActionMem token must be the valid ACTION_QUERY token "
    ...
    def _compute_action_token_objective(
        ...
        # it. Causal PI05ActionMem attention prevents this hidden state from seeing
        # the target token.
...
class PI05ActionMemPolicy(PreTrainedPolicy):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        # Keep token metadata next to the policy artifact. This also repairs a
        # stale absolute path when a checkpoint directory is moved to another
        # machine, while still allowing an explicit valid override.
        local_token_map = Path(pretrained_name_or_path).expanduser() / "token_map.json"
        if local_token_map.is_file():
            config.action_token_map_path = str(local_token_map.resolve())
        else:
            configured_token_map = getattr(config, "action_token_map_path", None)
            configured_token_map_exists = (
                configured_token_map is not None and Path(configured_token_map).expanduser().is_file()
            )
            if not configured_token_map_exists:
                try:
                    from transformers.utils import cached_file

                    config.action_token_map_path = cached_file(
                    ...
                    raise FileNotFoundError(
                        f"token_map.json was not found in PI05ActionMem model artifact "
                        f"{pretrained_name_or_path!r}."
                    ) from exc
