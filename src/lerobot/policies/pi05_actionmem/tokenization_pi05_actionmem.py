...
def default_action_token_map_path() -> Path:
    """Return the workspace-local PI05ActionMem token-map path."""
    return Path(__file__).resolve().parents[6] / "models" / "pi05_actionmem-base" / "token_map.json"


@dataclass(frozen=True)
class PI05ActionMemTokenMap:
    """Validated token IDs shared by the PI05ActionMem processor and model."""
    ...
    @classmethod
    def from_json(cls, path: str | Path | None = None) -> "PI05ActionMemTokenMap":
        resolved_path = Path(path or default_action_token_map_path()).expanduser().resolve()
        if not resolved_path.is_file():
            raise FileNotFoundError(f"PI05ActionMem token map does not exist: {resolved_path}")
    ...
    def _validate(self) -> None:
        ...
        if self.codebook_size != expected_codebook_size:
            raise ValueError(
                "Invalid PI05ActionMem token map: codebook_size does not match the configured code ID range "
        ...
        if mapped_min != self.token_id_min or mapped_max != self.token_id_max:
            raise ValueError(
                "Invalid PI05ActionMem token map: q0 mapping formula does not match token ID bounds."
            )
        ...
        if len(control_ids) != 3:
            raise ValueError("Invalid PI05ActionMem token map: control token IDs must be distinct.")
