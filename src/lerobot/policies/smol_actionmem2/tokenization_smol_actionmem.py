...
"""VQ metadata and local action-code IDs for Smol ActionMem 2."""
...
def default_smol_actionmem2_token_map_path() -> Path:
    return Path(__file__).resolve().parents[6] / "models" / "smol_actionmem2-base" / "token_map.json"


@dataclass(frozen=True)
class SmolActionMem2TokenMap:
    ...
    @classmethod
    def from_json(cls, path: str | Path | None = None) -> SmolActionMem2TokenMap:
        resolved_path = Path(path or default_smol_actionmem2_token_map_path()).expanduser().resolve()
        if not resolved_path.is_file():
            raise FileNotFoundError(f"Smol ActionMem 2 token map does not exist: {resolved_path}")
    ...
    def _validate(self) -> None:
        ...
        if self.codebook_size != expected_size:
            raise ValueError(
                "Invalid Smol ActionMem 2 token map: codebook_size does not match the q0 range "
                f"({self.codebook_size} != {expected_size})."
            )
        if self.codebook_size <= 0:
            raise ValueError("Smol ActionMem 2 requires a non-empty q0 codebook.")
