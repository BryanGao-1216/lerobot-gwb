...
class SmolActionMem2FlowMatching(VLAFlowMatching):
    """SmolVLA flow expert with an independent discrete action vocabulary."""
    ...
    def __init__(self, config: SmolActionMem2Config, rtc_processor=None):
        ...
        if self._normalize_action_vqvae_flow_source and mean is not None:
            if len(mean) != self.action_code_map.action_dim:
                raise ValueError(
                    f"Action normalization stats have dimension {len(mean)}, but the VQ-VAE action "
                    f"dimension is {self.action_code_map.action_dim}."
                )
        ...
        checkpoint_path = config.action_vqvae_checkpoint_path or self.action_code_map.vqvae_checkpoint_path
    ...
    def _get_action_vqvae(self, device: torch.device) -> ActionVQVAEQ0Decoder:
        if self.action_vqvae_checkpoint_path is None:
            raise ValueError(
                "Set action_vqvae_checkpoint_path or vqvae.checkpoint_path in the Smol ActionMem 2 "
        ...
        if action_vqvae is None:
            ...
            if action_vqvae.action_dim != self.action_code_map.action_dim:
                raise ValueError(
                    f"Action VQ-VAE action_dim ({action_vqvae.action_dim}) does not match "
                    f"the token map ({self.action_code_map.action_dim})."
                )
            if action_vqvae.codebook_size != self.action_code_map.codebook_size:
                raise ValueError(
                    f"Action VQ-VAE codebook_size ({action_vqvae.codebook_size}) does not match "
                    f"the token map ({self.action_code_map.codebook_size})."
                )
    ...
    def decode_action_classes(
        ...
        invalid = (action_classes < self.action_code_map.action_class_min) | (
            action_classes > self.action_code_map.action_class_max
        )
        if torch.any(invalid):
            invalid_ids = action_classes[invalid].detach().cpu().tolist()
            raise ValueError(
                f"Action classes must be in [{self.action_code_map.action_class_min}, "
                f"{self.action_code_map.action_class_max}], got {invalid_ids}."
            )
        ...
        if self._normalize_action_vqvae_flow_source:
            if self._action_vqvae_flow_mean is None or self._action_vqvae_flow_std is None:
                raise ValueError(
                    "Smol ActionMem 2 uses MEAN_STD action normalization, but flow-source mean/std "
    ...
    def sample_actions(
        ...
        **kwargs: Unpack[ActionSelectKwargs],
        ...
        if noise is None:
            noise = self.decode_action_classes(generated_action_token)
