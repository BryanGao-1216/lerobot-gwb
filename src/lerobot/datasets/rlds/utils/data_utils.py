...
def get_dataset_statistics(
    ...
    # diter = dataset.iterator()
    # for d_idx in tqdm(range(cardinality if cardinality != tf.data.UNKNOWN_CARDINALITY else 0)):
    #     try:
    #         traj = diter.next()
    ...
    for traj in tqdm(
        dataset.iterator(), total=cardinality if cardinality != tf.data.UNKNOWN_CARDINALITY else None
    ):
        actions.append(traj["action"])
        proprios.append(traj["proprio"])
        num_transitions += traj["action"].shape[0]
