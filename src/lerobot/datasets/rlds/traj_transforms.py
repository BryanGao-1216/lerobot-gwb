...
def chunk_act(traj: Dict, window_size: int, future_action_window_size: int = 0) -> Dict:
    """
...
def add_pad_mask_dict(traj: Dict) -> Dict:
    ...
    # when only load action, there is no observation or task
    if "observation" not in traj and "task" not in traj:
        return traj
