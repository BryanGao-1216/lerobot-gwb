...
def make_action_dataset_from_rlds(
    ...
    *,
    ...
    """
    Simplified dataset loading function that only retains the "action" and "dataset_name" keys.
    ...
    Returns:
        Dataset: A dataset containing only the "action" and "dataset_name" keys.
    """
    ...
    *,
    ...
    dataset = dataset.frame_map(
        partial(
            apply_obs_transform,
            partial(
                obs_transforms.decode_and_resize, resize_size=resize_size, depth_resize_size=depth_resize_size
            ),
        ),
        num_parallel_calls,
    )
...
def make_interleaved_action_dataset(
    dataset_kwargs_list: List[Dict],
    sample_weights: Optional[List[float]] = None,
    *,
    train: bool,
    ...
    """
    Creates an interleaved action sequence dataset from list of dataset configs (kwargs). Returns a dataset of batched frames.
    ...
    for dataset_kwargs in dataset_kwargs_list:
        ...
        if "dataset_frame_transform_kwargs" in data_kwargs:
            data_kwargs.pop("dataset_frame_transform_kwargs")
        if only_action:
            _, dataset_statistics = make_action_dataset_from_rlds(**data_kwargs, train=train)
        else:
            _, dataset_statistics = make_dataset_from_rlds(**data_kwargs, train=train)
        dataset_sizes.append(dataset_statistics["num_transitions"])
        all_dataset_statistics[dataset_kwargs["name"]] = dataset_statistics
    ...
    for dataset_kwargs, threads, reads in zip(
        ...
        if "dataset_frame_transform_kwargs" in dataset_kwargs:
            dataset_kwargs.pop("dataset_frame_transform_kwargs")
        if only_action:
            dataset, _ = make_action_dataset_from_rlds(
                **dataset_kwargs,
                train=train,
                num_parallel_calls=threads,
                num_parallel_reads=reads,
